...
const ProfilePage = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { error: profileError } = await supabase
      .from("profiles")
      .update({ display_name: trimmedName || null } as any)
      .eq("user_id", user.id);
    ...
    if (trimmedEmail !== user.email) {
      ...
      toast.success("Profile saved! Check your new email to confirm the change.");
    } else {
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="container mx-auto flex items-center h-16 px-4 gap-3">
          ...
          <h1 className="font-heading font-bold text-lg">Profile Settings</h1>
        </div>
      </header>
          ...
          <form onSubmit={handleSave} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-11"
              />
              <p className="text-xs text-muted-foreground">How you want to be addressed in the app.</p>
            </div>
